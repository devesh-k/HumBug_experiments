    ...
    def forward(self, x,train = True):
        ...
        with torch.no_grad():
            if train == True:
                ...
                ta_transformations_rndm_choice = VT.RandomChoice([AT.FrequencyMasking(freq_mask_param=100),AT.TimeMasking(time_mask_param=50)], p=[.45, .45])
    ...
    for e in range(num_epochs + cooldown_epoch):
        ...
        for batch_i, inputs in enumerate(train_loader):
            ...
            with autocast():
                ...
                #y_pred_smax = softmax(y_pred)
                preds = torch.argmax(y_pred, axis = 1)
                loss = criterion_1(y_pred, y.unsqueeze(dim=1))
