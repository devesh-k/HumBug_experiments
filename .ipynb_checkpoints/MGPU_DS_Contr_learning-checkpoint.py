...
def add_argument():
    ...
    parser.add_argument('--per_device_train_batch_size', type=int, default=64,
                        help='batch_size for per device training')
...
## Train_model ####
#trainloader, model_engine,classes,device,e ,base_optimiser, fp16,encoder
def train_model(train_loader, model ,classes,device,e,base_optimiser, fp16,criterion):
    ...
    #criterion = SupConLoss(temperature= .07)
    criterion = criterion
    all_train_loss = []
...
for e in range(num_epochs + cooldown_epoch):
    ...
    train_f1,train_loss = train_model(trainloader, model_engine,classes,device,e ,base_optimiser, fp16,criterion = SupConLoss(temperature= .07))
    dist.barrier()
    ...
    all_train_f1.append(train_f1)
    val_loss, val_f1 , _,_ = test_model(model_engine, val_loader, device , fp16 ,criterion = nn.CrossEntropyLoss(), call = "val")
