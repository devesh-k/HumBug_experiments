...
class SpecAug_All(nn.Module):
    def __init__(self):
        ...
        self.transformations = nn.Sequential(
            VT.transforms.RandomApply([VT.GaussianBlur(3),VT.RandomErasing(),torchaudio.transforms.FrequencyMasking(freq_mask_param=100),torchaudio.transforms.TimeMasking(time_mask_param=50)             
...
class FreqMask(nn.Module):
    def __init__(self):
        ...
        self.transformations = nn.Sequential(
            VT.transforms.RandomApply([VT.GaussianBlur(3),
            VT.RandomErasing(),torchaudio.transforms.FrequencyMasking(freq_mask_param=80),
...
class TimeMask(nn.Module):
    def __init__(self):
        ...
        self.transformations = nn.Sequential(
            VT.transforms.RandomApply([VT.GaussianBlur(3),
            VT.RandomErasing(), torchaudio.transforms.TimeMasking(time_mask_param=25)
                ], p=0.40),  # Example probability of applying the transformations
            # Additional transformations or layers can be added here
        )
...
def train_model(train_loader, val_loader,test_loader, model, classes ,df,num_epochs = args.num_epochs ,n_channels = 1):
    ...
    scheduler = timm.scheduler.CosineLRScheduler(base_optimiser, t_initial=num_epochs/2,lr_min= 1e-6)
