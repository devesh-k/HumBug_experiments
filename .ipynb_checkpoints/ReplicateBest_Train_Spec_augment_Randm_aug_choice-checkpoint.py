...
class SpecAug_All(nn.Module):
    def __init__(self):
        ...
        self.transformations = nn.Sequential(
            ...
            ], p=0.5),  # Example probability of applying the transformations
...
class TimeMask(nn.Module):
    def __init__(self):
        ...
        self.transformations = nn.Sequential(
            VT.transforms.RandomApply([VT.GaussianBlur(3),
            VT.RandomErasing(), torchaudio.transforms.TimeMasking(time_mask_param=50)
                ], p=0.50),  # Example probability of applying the transformations
            # Additional transformations or layers can be added here
        )
...
class MyModel(nn.Module):
    ...
    def forward(self, input_ids,train = True ,attention_mask = False):
        ...
        if train== True:
            aug_choice = [1,2,3]
            choice = random.choice(aug_choice)
            if choice == 1:
