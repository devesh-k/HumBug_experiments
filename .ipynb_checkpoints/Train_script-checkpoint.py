...
parser = argparse.ArgumentParser(description='Trainable_Mel_layer',
...
def train_model(train_loader, val_loader,test_loader, model ,class_weights, classes = classes, num_epochs = args.num_epochs ,n_channels = 1):
...
if __name__ == '__main__':
    ...
    classes = ['an arabiensis','culex pipiens complex', 'ae aegypti','an funestus ss','an squamosus',
               'an coustani','ma uniformis','ma africanus' ]
    df = prepare_df(classes = classes,csv_loc = config.data_df)
    plot_df(df)
    ...
    tr_model, lr_log,all_train_f1,all_train_loss,all_val_loss,all_val_f1 = train_model(train_loader, val_loader, test_loader,model, class_weights ,classes ,num_epochs = num_epochs )
