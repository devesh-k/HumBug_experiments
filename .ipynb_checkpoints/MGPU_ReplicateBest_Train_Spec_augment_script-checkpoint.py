    ...
    def __init__(self):
        ...
        self.transformations = nn.Sequential(
            VT.transforms.RandomApply([
                ...
                ], p=0.70),  # Example probability of applying the transformations
            # Additional transformations or layers can be added here
        )
    ...
    def __init__(self):
        ...
        self.transformations = nn.Sequential(
            VT.transforms.RandomApply([
                ...
                ], p=0.60),  # Example probability of applying the transformations
            # Additional transformations or layers can be added here
        )
...
## Train_model ####
#train_loader, val_loader, test_loader,model,classes,df_train_offset ,args.num_epochs,device,train_sampler 
def train_model(train_loader, val_loader,test_loader, model, classes ,df,num_epochs,train_sampler):
...
def worker(local_rank, args):
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, num_workers=num_workers,batch_size = batch_size, pin_memory=True,drop_last = True ,sampler=train_sampler)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size,num_workers=num_workers, pin_memory=pin_memory,drop_last = True ,sampler = val_sampler)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=batch_size,num_workers= num_workers, pin_memory=pin_memory,drop_last = True ,sampler = test_sampler)
    ...
    tr_model, lr_log,all_train_f1,all_train_loss,all_val_loss,all_val_f1 = train_model(train_loader, val_loader, test_loader,model,classes,df_train_offset ,args.num_epochs,train_sampler )
