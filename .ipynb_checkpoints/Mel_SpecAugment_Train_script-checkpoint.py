...
#             print("out_smax shape = " , out_smax.shape)
        ...
        out = torch.argmax(backbone_op , dim = 1)
        #print("out = ",out)
        
        output_dict['probs'] = backbone_op
        output_dict['preds'] = out
        #print("^^^^^ inside forward^^^^^^^")
        del spec_gram,backbone_op
        if DEBUG:
...
def train_model(train_loader, val_loader,test_loader, model, classes ,df,num_epochs = args.num_epochs ,n_channels = 1):
    ...
    base_optimiser = timm.optim.AdamP(model.parameters(), lr= 1e-5)
    ...
    scheduler = timm.scheduler.CosineLRScheduler(base_optimiser, t_initial=num_epochs/2,lr_min= 1e-7,cycle_decay = .90,warmup_t = 5 ,warmup_lr_init = 1e-6)
