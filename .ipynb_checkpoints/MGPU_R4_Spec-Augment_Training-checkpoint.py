...
class MyModel(nn.Module):
    def __init__(self, model_name, image_size,device):
        ...
        self.spec_layer = features.STFT(n_fft=int(config.NFFT), freq_bins=None, hop_length=int(config.n_hop),
                           ...
                           sr=config.rate, output_format="Magnitude", trainable=False,verbose = False).to(device)
...
def test_model(model, loader, device,criterion, call = "val"):
    ...
    with torch.no_grad():
        ...
        test_f1 = torch.tensor(test_f1).to(device)
        test_f1 = test_f1.to(dtype=torch.float32)  # Make sure the tensor is of float32 data type
        test_f1 = test_f1.contiguous()  # Make sure the tensor is stored in contiguous memory
        dist.all_reduce(test_f1, op=dist.ReduceOp.AVG)
...
def train_model(train_loader, val_loader,test_loader, model ,classes,class_weights,num_epochs,train_sampler,device,global_rank ):
...
#         model = nn.DataParallel(model, device_ids=list(range(torch.cuda.device_count())))
    ...
    for e in range(num_epochs + cooldown_epoch):
        ...
        train_f1 = torch.tensor(train_f1).to(device)
        train_f1 = train_f1.to(dtype=torch.float32)  # Make sure the tensor is of float32 data type
        train_f1 = train_f1.contiguous()  # Make sure the tensor is stored in contiguous memory
        
        #f1_tensor = torch.tensor(train_f1).to('cuda')
        dist.all_reduce(train_f1, op=dist.ReduceOp.AVG)

        # Calcul    ate average F1 score across all processes
...
def worker(local_rank, args):
    ...
    #model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(MyModel(8)).to(device)
    model.to(device)
    ...
    tr_model, lr_log,all_train_f1,all_train_loss,all_val_loss,all_val_f1 = train_model(train_loader, val_loader, test_loader,model,classes,class_weights,num_epochs ,train_sampler,device,global_rank )
    print("ALL DONE!!!!")
