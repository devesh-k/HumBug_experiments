...
def train_model(train_loader, model,classes,class_weights,train_sampler,device,e ,base_optimiser , look_optimiser,scheduler ):
...
def worker(local_rank, args):
    ...
    for e in range(num_epochs + cooldown_epoch):
        ...
        train_f1,train_loss = train_model(train_loader, model,classes,class_weights,train_sampler,device,e ,base_optimiser ,look_optimiser,scheduler )
        dist.barrier()
